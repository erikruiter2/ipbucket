...
def db_delete_entry(tablename, **kwargs):
    """ Deletes an entry from a table, returns an errormessage on failure, on success it returns True"""

    try:
        entry = tablename.query.filter_by(**kwargs).first()
        if entry:
        ...
        return "Error while deleting entry: " + str(e)
    except TypeError as e:
        return "Error while deleting entry: " + str(e) 
    except IntegrityError as e:
        db_session.rollback()
        return "Error while deleting entry: " + str(e.orig.message) + str(e.params)
    return True
