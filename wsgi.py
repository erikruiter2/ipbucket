...
import logging
import json
from database import init_db, db_session, db_query_by_id, db_add_entry, db_delete_entry, db_query_all
...
@wsgi_app.route('/api/ip_domain/<int:ip_domain_id>', methods=['DELETE'])
def api_ip_domain_delete(ip_domain_id):
    result = db_delete_entry(IPDomain, id=ip_domain_id)
    if type(result) is str:
        return response('error', result)
    else:
        return response('success', 'IP Domain entry ' + str(ip_domain_id) + ' succesfully deleted.')


@wsgi_app.route('/api/ip_address/<int:ip_domain_id>/<int:ip>', methods=['DELETE'])
def api_ip_address_delete(ip_domain_id, ip):
    result = db_delete_entry(IPv4Address, ip=ip, ip_domain_id=ip_domain_id)
    if type(result) is str:
        return response('error', result)
    else:
        return response('success', 'IP Address entry ' + long2ip(ip) + ' succesfully deleted.')
